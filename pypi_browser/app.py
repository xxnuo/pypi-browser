...
TEXT_RENDER_FILESIZE_LIMIT = int(
    os.environ.get("PYPI_BROWSER_TEXT_RENDER_FILESIZE_LIMIT", ONE_MB)
)
...
async def package(request: Request) -> Response:
    ...
    if package_name != normalized_package_name:
        return RedirectResponse(
            request.url_for("package", package=normalized_package_name)
        )
    ...
    try:
        ...
        return templates.TemplateResponse(
            "package.html",
            {
                ...
                "total_files": len(
                    set(itertools.chain.from_iterable(version_to_files.values()))
                ),
            },
        )
    ...
    if len(metadata_entries) > 0:
        ...
        async with package.open_from_archive(metadata_path) as f:
            metadata_file = io.StringIO(
                (await f.read()).decode("utf8", errors="ignore")
            )

        message = email.message_from_file(metadata_file)
        metadata = {key: message.get_all(key) for key in set(message.keys())}
    else:
        metadata_path = None
        metadata = {}
...
async def package_file_archive_path(request: Request) -> Response:
    ...
    def _transfer_raw() -> Response:
        ...
        return StreamingResponse(
            transfer_file(),
            media_type=mimetype
            if (mimetype or "").startswith(MIME_WHITELIST)
            else None,
        ...
        if entry.size <= TEXT_RENDER_FILESIZE_LIMIT:
            ...
            return templates.TemplateResponse(
                "package_file_archive_path.html",
                {
                    "request": request,
                    "package": package_name,
                    "package_is_tarball": package.package_format
                    is PackageFormat.TARBALL,
                    "filename": file_name,
                    "archive_path": archive_path,
...
async def search(request: Request) -> Response:
    return RedirectResponse(
        request.url_for("package", package=request.query_params["package"])
    )
...
app = Starlette(
    ...
    routes=[
        ...
        Route(
            "/package/{package}/{filename}/{archive_path:path}",
            endpoint=package_file_archive_path,
        ),
...
app.mount(
    "/static",
    StaticFiles(directory=os.path.join(install_root, "static")),
    name="static",
)
